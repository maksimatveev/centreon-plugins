...
package network::huawei::standatd::mode::memory;
...
sub run {
    ...
    my $oid_hwMemoryDevSize = '.1.3.6.1.4.1.2011.6.3.5.1.1.2';
    ...
    my $oid_hwMemoryDevRawSliceUsed = '.1.3.6.1.4.1.2011.6.3.5.1.1.4';
    ...
    my $oid_hwMemoryDevFree = '.1.3.6.1.4.1.2011.6.3.5.1.1.3';
    ...
    foreach my $oid (keys %$result) {
        ...
        $oid =~ /\.([0-9]+)$/;
        my $instance = $1;
        my $memory_used = $result->{$oid_hwMemoryDevRawSliceUsed . '.' . $instance};
        my $memory_free = $result->{$oid_hwMemoryDevFree . '.' . $instance};
        my $total_size = $result->{$oid_hwMemoryDevSize . '.' . $instance};
...
Check memory usage (HUAWEI-MEMORY-MIB).
